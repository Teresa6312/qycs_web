...
def create_user(strategy, details, backend, user=None, *args, **kwargs):
    isCurrentUser=None
    try:
        isCurrentUser=User.objects.get(email=details['email'])
    except:
        pass
    if not user and isCurrentUser:
        messages.error(kwargs['request'],_('You already have an account with this email. Please use the below username and password fields to log in.'))
        return {'is_new': False}
