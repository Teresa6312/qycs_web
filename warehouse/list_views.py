...
class ReadyDirectPackages(TemplateView):
	...
	def get(self, request):
		if request.user.is_staff or request.user.is_superuser:
			packages = Service.objects.filter(co_shipping = False, parent_package__shipped_date = None).exclude(paid_amount=None).order_by('-parent_package','cust_tracking_num')
